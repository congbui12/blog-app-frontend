...
const Dashboard = () => {
  ...
  //   const res =
  //     view === 'myPosts'
  //       ? await userAPI.fetchMyPostsAPI(sortedBy, page, 3)
  //       : await userAPI.fetchFavoritePostsAPI(page, 3);
  //   return res;
  // };
  ...
  // if (isError)
  //   return <p className="text-center font-bold text-gray-700">Error: {error?.message}</p>;
  ...
  return (
    <Layout>
      Dashboard
      {/* <Link
        to="/protected/post/new"
        state={{ from: location }}
        className="block text-center text-lg font-semibold text-gray-700 md:text-end hover:text-blue-500 mb-4"
      >
        Share your story✍️
      </Link>
      <div className="flex flex-col items-center justify-center gap-2 md:flex-row mb-4">
        {/* Buttons to switch between components */}
      {/* <button onClick={() => handleViewChange('myPosts')} className={viewClass('myPosts')}>
          My posts
        </button>
        <button
          onClick={() => handleViewChange('favoritePosts')}
          className={viewClass('favoritePosts')}
        >
          My favorite posts
        </button>
      </div>
      <div>
        {view === 'myPosts' && (
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-4">
            <label htmlFor="sort-select" className="font-medium text-gray-700">
              Sorted by:
            </label>
            <select
              value={sortedBy}
        ...
        {isLoading && (
          <p className="mt-4 text-center font-bold text-gray-700">Loading more posts...</p>
        )}
